...
ALTER TABLE `stores` ADD CONSTRAINT `fk_store_dept` FOREIGN KEY (`department_id`) REFERENCES `departments` (`id`) ON DELETE SET NULL;

-- 评论表增加重要性等级
ALTER TABLE `reviews` ADD COLUMN IF NOT EXISTS `importance_level` ENUM('high', 'medium', 'low') DEFAULT 'medium' COMMENT '重要性等级' AFTER `status`;
